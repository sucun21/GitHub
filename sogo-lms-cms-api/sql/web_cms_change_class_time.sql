...
DROP PROCEDURE IF EXISTS web_cms_change_class_time;
//

CREATE PROCEDURE `web_cms_change_class_time`(
	p_is_admin BIT,
	p_class_id INT,
    p_trainer_id INT,
    p_new_class_time TIMESTAMP
)
BEGIN
	...
	ELSEIF p_new_class_time IS NULL OR p_new_class_time <= NOW() THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid class time';
	END IF;
    
    UPDATE sg_offline_class
    SET study_date = DATE(p_new_class_time),
		study_time = TIME(p_new_class_time)
    WHERE class_id = p_class_id;
    
    SELECT 1 `id`,
			  'Class time changed' `message`;
END
//
